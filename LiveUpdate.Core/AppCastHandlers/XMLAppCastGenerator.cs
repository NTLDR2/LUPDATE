...
namespace LiveUpdate.Engine.AppCastHandlers
{
    ...
    public class XMLAppCastGenerator : IAppCastGenerator
    {
        ...
        // =================================================================
        // [LUPDATE EXTENSION] Custom XML namespace for asset manifests.
        // This namespace is used in <lupdate:assets> elements within AppCast
        // XML to define per-file download manifests for granular updates.
        // =================================================================
        /// <summary>
        /// LUPDATE XML namespace for custom extensions (asset manifests, etc.)
        /// </summary>
        public static readonly XNamespace LupdateNamespace = "http://lupdate.local/appcast";
        ...
        public AppCastItem ReadAppCastItem(XElement item)
        {
            ...
            // =================================================================
            // [LUPDATE EXTENSION] Parse <lupdate:assets> element if present.
            // This enables per-file granular/delta updates. Each <lupdate:asset>
            // child element describes a single file to download individually.
            //
            // XML format:
            //   <lupdate:assets xmlns:lupdate="http://lupdate.local/appcast">
            //     <lupdate:asset path="SPYSCALP.exe"
            //                    url="https://server.com/v3/SPYSCALP.exe"
            //                    hash="abc123..." size="2048000" />
            //   </lupdate:assets>
            //
            // When assets are present, AssetManifestInstaller handles the
            // update instead of the single-file download pipeline.
            // =================================================================
            var assetsElement = item.Element(LupdateNamespace + "assets");
            if (assetsElement != null)
            {
                var assetElements = assetsElement.Elements(LupdateNamespace + "asset");
                foreach (var assetEl in assetElements)
                {
                    var asset = new UpdateAsset()
                    {
                        RelativePath = assetEl.Attribute("path")?.Value ?? string.Empty,
                        DownloadUrl = assetEl.Attribute("url")?.Value ?? string.Empty,
                        Hash = assetEl.Attribute("hash")?.Value ?? string.Empty,
                        Size = 0
                    };
                    string sizeStr = assetEl.Attribute("size")?.Value ?? "";
                    if (long.TryParse(sizeStr, out var assetSize))
                    {
                        asset.Size = assetSize;
                    }
                    if (!string.IsNullOrWhiteSpace(asset.RelativePath) && !string.IsNullOrWhiteSpace(asset.DownloadUrl))
                    {
                        newAppCastItem.Assets.Add(asset);
                        _logWriter?.PrintMessage("  Parsed asset: {0} ({1} bytes)", asset.RelativePath, asset.Size.ToString());
                    }
                }
                _logWriter?.PrintMessage("  Total assets in manifest: {0}", newAppCastItem.Assets.Count.ToString());
            }
        ...
        public static XElement GetXElement(AppCastItem appCastItem, string signatureAttribute = "")
        {
            ...
            // =================================================================
            // [LUPDATE EXTENSION] Serialize <lupdate:assets> if present.
            // Writes the asset manifest as child elements of the <item>.
            // =================================================================
            if (appCastItem.HasAssetManifest)
            {
                var assetsEl = new XElement(LupdateNamespace + "assets");
                foreach (var asset in appCastItem.Assets)
                {
                    var assetEl = new XElement(LupdateNamespace + "asset");
                    assetEl.Add(new XAttribute("path", asset.RelativePath ?? ""));
                    assetEl.Add(new XAttribute("url", asset.DownloadUrl ?? ""));
                    if (!string.IsNullOrWhiteSpace(asset.Hash))
                    {
                        assetEl.Add(new XAttribute("hash", asset.Hash));
                    }
                    assetEl.Add(new XAttribute("size", asset.Size));
                    assetsEl.Add(assetEl);
                }
                item.Add(assetsEl);
            }
        ...
        public XDocument GenerateAppCastXml(List<AppCastItem> appCastItems, string? title, string? link = "", string? description = "", string? language = "en")
        {
            ...
            var document = new XDocument(
                new XElement("rss", new XAttribute("version", "2.0"),
                    new XAttribute(XNamespace.Xmlns + "sparkle", SparkleNamespace),
                    new XAttribute(XNamespace.Xmlns + "lupdate", LupdateNamespace),  // [LUPDATE EXTENSION] Add lupdate namespace to RSS root
                    channel)
            );
