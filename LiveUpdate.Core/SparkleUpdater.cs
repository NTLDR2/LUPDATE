...
using LiveUpdate.Engine.Events;
...
using LiveUpdate.Engine.Installers;
using System.Text;
...
namespace LiveUpdate.Engine
{
    ...
    public partial class SparkleUpdater : IDisposable
    {
        ...
        protected virtual string GetWindowsInstallerCommand(string downloadFilePath)
        {
            ...
            // [LUPDATE EXTENSION] ZIP bundle support on Windows.
            // Instead of a shell command, return a sentinel value that
            // RunDownloadedInstaller() will intercept and route through
            // ZipUpdateInstaller.ExtractAndReplace() instead.
            if (DoExtensionsMatch(installerExt, ".zip"))
            {
                return ZipUpdateInstaller.ZipExtractSentinel;
            }
            return downloadFilePath;
        }
        ...
        protected virtual async Task RunDownloadedInstaller(string downloadFilePath)
        {
            ...
            // =================================================================
            // [LUPDATE EXTENSION] ZIP Bundle Handler
            // If the download is a ZIP file, we skip the batch-file installer
            // flow entirely and instead extract the ZIP directly to the
            // application's install directory. This supports apps distributed
            // as loose binaries (EXE + DLLs) rather than MSI packages.
            // =================================================================
            string installerCmdCheck;
            try { installerCmdCheck = GetInstallerCommand(downloadFilePath); }
            catch { installerCmdCheck = ""; }

            if (installerCmdCheck == ZipUpdateInstaller.ZipExtractSentinel)
            {
                LogWriter?.PrintMessage("ZIP update detected — routing through ZipUpdateInstaller");
                // Determine the target installation directory.
                // RestartExecutablePath is the app's working directory (set by the host app).
                // Falls back to the directory of the executable or the app domain base.
                string targetDir = RestartExecutablePath;
                if (string.IsNullOrWhiteSpace(targetDir))
                {
                    targetDir = Path.GetDirectoryName(RestartExecutableName) ?? AppDomain.CurrentDomain.BaseDirectory;
                }
                bool success = ZipUpdateInstaller.ExtractAndReplace(downloadFilePath, targetDir, LogWriter);
                if (success)
                {
                    LogWriter?.PrintMessage("ZIP extraction succeeded. Requesting application close for restart.");
                    // Signal the host application to close so the updated binaries take effect.
                    // The host app hooks into this event to perform graceful shutdown.
                    CloseApplicationAsync?.Invoke();
                }
                else
                {
                    LogWriter?.PrintMessage("ZIP extraction FAILED. Rollback attempted by ZipUpdateInstaller.");
                    InstallUpdateFailed?.Invoke(InstallUpdateFailureReason.CouldNotBuildInstallerCommand, downloadFilePath);
                }
                return; // Skip the normal batch-file installer flow
            }
            // =================================================================
            // END LUPDATE ZIP EXTENSION — normal installer flow continues below
            // =================================================================
