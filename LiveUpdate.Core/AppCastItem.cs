...
namespace LiveUpdate.Engine
{
    ...
    public class AppCastItem : IComparable<AppCastItem>
    {
        ...
        // =====================================================================
        // [LUPDATE EXTENSION] Asset Manifest for Granular/Delta Updates
        // =====================================================================
        //
        // Standard AppCast items have a single <enclosure> download URL (one file).
        // The asset manifest extends this with individual file entries, enabling:
        //   - Delta updates (download only changed files)
        //   - Bandwidth savings (skip unchanged DLLs)
        //   - Targeted updates (update a plugin without touching the host)
        //
        // When HasAssetManifest is true, LUPDATE uses AssetManifestInstaller
        // instead of the normal single-file download pipeline.
        // The <enclosure> URL serves as a ZIP fallback if any asset fails.
        // =====================================================================

        /// <summary>
        /// Optional list of individual asset files for granular/delta updates.
        /// When populated, LUPDATE downloads each asset individually instead of
        /// the single file specified in the <enclosure> element's download URL.
        /// 
        /// Parsed from <lupdate:assets> elements in the AppCast XML.
        /// If empty, the standard single-file download (MSI/ZIP/EXE) is used.
        /// </summary>
        [JsonPropertyName("assets")]
        public List<UpdateAsset> Assets { get; set; } = new List<UpdateAsset>();

        /// <summary>
        /// True if this AppCast item has an asset manifest (per-file download mode).
        /// When true, AssetManifestInstaller handles the update instead of
        /// the normal download-and-run-installer pipeline.
        /// </summary>
        [JsonIgnore]
        public bool HasAssetManifest => Assets != null && Assets.Count > 0;
